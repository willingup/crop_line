...
int main()
{
    ...
    //original image
    Mat src = cv::imread("/home/robot/pictures/auto_pictures6/139.png");
    ...
    Mat roi=src(Rect(width/3,100,width/3,height-150));
       ...
       for (int index = 0; index >= 0; index = hierarchy[index][0]) {
           ...
           if (pt2_center.x<image_cut.cols/2+w)
           {
               if(image_cut.cols/2-w < pt2_center.x){
                    ...
                    cout << "目前centers的大小" << endl << centers_size << endl;
                    
                    
                    //last_point.x= centers[centers_size-1].x;
                    //cout << "上一个点的坐标" << endl << last_point.x<< endl;
                    if(centers_size>1)
                    {
