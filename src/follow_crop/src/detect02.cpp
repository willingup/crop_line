...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "detect_crop_line02");
    ros::NodeHandle n;
    LineDetect det;
    ...
    //ros::Publisher imgPub = n.advertise<>("process_after_img", 1);
    ...
    while (ros::ok()) {
        if (!det.src.empty()) {
            ...
            //ROI image
            int src_width = det.src.cols;
            int src_height = det.src.rows;
            int roi_top = 50;
            int roi_height = src_height-150;
            det.roi = det.src(Rect(src_width/4,roi_top,src_width/2,roi_height));
            ...
            for (int j = 0;j < centers.size();j++)
	        {
                ...
                img_center_point.x = det.roi.cols/2;
